...
$(function(){
	...
	function doConversion()
	{
		...
		{
			$('#output, #output2').html(formattedEmptyOutputMsg);
			return;
		}

		let output = jsonToGo(input, "");

		if (output.error)
		{
			$('#output, #output2').html('<span class="clr-red">'+output.error+'</span>');
			...
			if (parsedError) {
				try {
					...
					faultyIndex && $('#output, #output2').html(constructJSONErrorHTML(output.error, faultyIndex, input));
				} catch(e) {}
			}
		}
		else
		{
			...
			$('#output2').text(finalOutput.replace(/data class/g, "export type").replace(/val /g, "").replace(/\t@JsonProperty\(.*\n/g, "").replace(/\)/g, "};").replace(/\(/g, " {").replace(/(List)<(.*)>/g, "$2[]").replace(/: String/g, ": string").replace(/: Int|: Float/g, ": number").replace(/: Any/g, ": any"));
	...
	$('#output').keyup(function()
	{
		...
		$('#output2').text(text.replace(/data class/g, "export type").replace(/val /g, "").replace(/\t@JsonProperty\(.*\n/g, "").replace(/\)/g, "};").replace(/\(/g, " {").replace(/(List)<(.*)>/g, "$2[]").replace(/: String/g, ": string").replace(/: Int|: Float/g, ": number").replace(/: Any/g, ": any"));
		mainHighlight("ts");
	});

	// Highlights the output for the user
	// $('#output, #output2').click(function()
	// {
